...
// Mr. Sterling — stern, formal business boss
export default function SterlingCharacter({ audioVolume, isSpeaking, emotion }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    // Slow rare blink (every 5 s)
    const blinkPhase = ti % cfg.blinkInterval;
    const blinkVal = blinkPhase < 0.07
      ? blinkPhase / 0.07
      : blinkPhase < 0.14
      ? (0.14 - blinkPhase) / 0.07
      : 0;
    ...
    // Almost no idle motion — very controlled
    if (rootRef.current) {
      rootRef.current.position.y = Math.sin(ti * cfg.idleSpeed) * cfg.idleAmp;
    }

    // Head — minimal controlled nod
    if (headRef.current) {
    ...
    // Authoritative pointing gesture
  ...
  const lighterSlate = "#CBD5E1";
  const gold = "#F59E0B";
  const white = "#FFFFFF";
  const dark = "#0F172A";
  const nearBlack = "#0a0a0f";
  const suit = "#1E293B";
  ...
  return (
    <group ref={rootRef} position={[0, -0.5, 0]}>
      ...
      <group position={[0, 0, 0]}>
        {/* Suit body */}
        <mesh scale={[1.02, 1.08, 0.88]}>
          <sphereGeometry args={[0.42, 32, 32]} />
          <meshStandardMaterial color={suit} roughness={0.7} />
        </mesh>
        {/* Shirt front */}
        <mesh position={[0, 0.06, 0.33]} scale={[0.7, 0.9, 0.3]}>
          <sphereGeometry args={[0.22, 20, 20]} />
          <meshStandardMaterial color={white} roughness={0.9} />
        </mesh>
        {/* Tie */}
        <group position={[0, -0.02, 0.37]}>
          <mesh scale={[1, 1.6, 0.15]}>
            <boxGeometry args={[0.06, 0.22, 0.04]} />
            <meshStandardMaterial color={gold} roughness={0.4} metalness={0.1} />
          </mesh>
          <mesh position={[0, -0.2, 0]} scale={[1.4, 1, 1]}>
            <boxGeometry args={[0.06, 0.06, 0.04]} />
            <meshStandardMaterial color={gold} roughness={0.4} metalness={0.1} />
          </mesh>
        </group>
        {/* Suit lapels */}
        <mesh position={[0.08, 0.18, 0.33]} rotation={[0, -0.3, 0.4]}>
        ...
        {/* Left arm (stiff, at side) */}
        <group position={[0.4, 0.04, 0.02]}>
          <mesh rotation={[0.05, 0, 0.15]}>
            <capsuleGeometry args={[0.062, 0.26, 8, 16]} />
            <meshStandardMaterial color={suit} roughness={0.7} />
          </mesh>
        </group>
      ...
      <group ref={headRef} position={[0, 0.65, 0]}>
        {/* Head — slightly wider/squarer for authority */}
        <mesh scale={[1.08, 1.0, 0.95]}>
          <sphereGeometry args={[0.37, 32, 32]} />
          <meshStandardMaterial color={slate} roughness={0.6} />
        </mesh>

        {/* Hair — slicked back, dark */}
        <mesh position={[0, 0.27, -0.1]} scale={[1.05, 0.38, 1]}>
          <sphereGeometry args={[0.37, 24, 12]} />
          <meshStandardMaterial color={dark} roughness={0.4} />
        </mesh>
        {/* Side part line */}
        <mesh position={[0.04, 0.35, 0.14]} rotation={[0.2, 0, 0]}>
          <boxGeometry args={[0.01, 0.02, 0.2]} />
          <meshStandardMaterial color={lighterSlate} />
        </mesh>

        {/* Thin stern mouth */}
        <group position={[0, -0.17, 0.34]}>
          <mesh>
            <boxGeometry args={[0.16, 0.022, 0.01]} />
            <meshStandardMaterial color={darkSlate} />
          </mesh>
        ...
        <group position={[0.15, 0.08, 0.31]}>
          ...
          <mesh position={[0.01, 0, 0.072]}>
            <sphereGeometry args={[0.06, 16, 16]} />
            <meshStandardMaterial color={dark} />
          </mesh>
          <mesh position={[0.014, 0, 0.098]}>
            <sphereGeometry args={[0.032, 12, 12]} />
            <meshStandardMaterial color={nearBlack} />
          </mesh>
          <mesh position={[0.028, 0.026, 0.108]}>
            <sphereGeometry args={[0.012, 8, 8]} />
            <meshStandardMaterial color={white} emissive={white} emissiveIntensity={0.6} />
          </mesh>
          {/* Stern heavy brow — furrowed inward */}
          <mesh position={[-0.01, 0.12, 0.07]} rotation={[0, 0, -0.35]}>
            <boxGeometry args={[0.13, 0.035, 0.02]} />
            <meshStandardMaterial color={dark} />
          </mesh>
          <mesh ref={lidLeftRef} position={[0, 0.05, 0.05]} scale={[1, 0.001, 1]}>
            <sphereGeometry args={[0.105, 20, 10, 0, Math.PI * 2, 0, Math.PI * 0.58]} />
        ...
        <group position={[-0.15, 0.08, 0.31]}>
          ...
          <mesh ref={lidRightRef} position={[0, 0.05, 0.05]} scale={[1, 0.001, 1]}>
            <sphereGeometry args={[0.105, 20, 10, 0, Math.PI * 2, 0, Math.PI * 0.58]} />
            <meshStandardMaterial color={slate} />
          </mesh>
        </group>

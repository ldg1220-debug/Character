...
// Sunny — Yoshi-style dinosaur character
export default function SunnyCharacter({ audioVolume, isSpeaking, emotion }: Props) {
  ...
  // Colors
  const green = "#5DBB63";
  const darkGreen = "#3A8C40";
  const lightGreen = "#7DD881";
  const orange = "#FF9800";
  const white = "#FAFFF8";
  const dark = "#1a1a1a";
  const rosy = "#FF8C8C";
  const mouthRed = "#CC2200";
    ...
    <group ref={rootRef} position={[0, -0.45, 0]}>
      ...
      <group ref={bodyRef} position={[0, 0, 0]}>
        ...
        <group ref={leftArmRef} position={[0.4, 0.12, 0.1]}>
          ...
          {/* Hand */}
          <mesh position={[0.07, -0.18, 0.04]}>
            <sphereGeometry args={[0.08, 16, 16]} />
            <meshStandardMaterial color={lightGreen} roughness={0.75} />
          </mesh>
        </group>
        ...
        {/* Left foot */}
        <mesh position={[0.2, -0.42, 0.14]} rotation={[0.35, 0, 0]}>
        ...
        <group position={[0.12, 0.1, -0.38]} rotation={[-0.5, 0.3, 0.2]}>
          <mesh>
            <cylinderGeometry args={[0.05, 0.025, 0.28, 12]} />
      ...
      <group ref={headRef} position={[0, 0.65, 0.05]}>
        ...
        {/* Dorsal bumps (Yoshi's back-of-head spines) */}
        <mesh position={[0.08, 0.36, -0.18]} rotation={[0.4, 0, -0.3]}>
          <sphereGeometry args={[0.07, 16, 16]} />
          <meshStandardMaterial color={darkGreen} roughness={0.8} />
        </mesh>
        <mesh position={[-0.04, 0.4, -0.22]} rotation={[0.3, 0, 0.2]}>
          <sphereGeometry args={[0.08, 16, 16]} />
          <meshStandardMaterial color={darkGreen} roughness={0.8} />
        </mesh>
        <mesh position={[-0.16, 0.33, -0.15]} rotation={[0.5, 0, 0.4]}>
          <sphereGeometry args={[0.06, 16, 16]} />
          <meshStandardMaterial color={darkGreen} roughness={0.8} />
        </mesh>

        {/* Snout — Yoshi's distinctive protruding nose */}
        <group position={[0, -0.1, 0.3]}>
          <mesh scale={[1.1, 0.75, 1.3]}>
            <sphereGeometry args={[0.22, 24, 20]} />
            <meshStandardMaterial color={lightGreen} roughness={0.75} />
          </mesh>
          {/* Nostrils */}
          <mesh position={[0.07, 0.06, 0.22]}>
        ...
        <group position={[0, -0.19, 0.47]}>
          ...
          <mesh>
            ...
            <meshStandardMaterial color={mouthRed} />
          </mesh>
          {/* Mouth interior (appears when speaking) */}
        ...
        <group position={[0.19, 0.17, 0.31]}>
          ...
          {/* Iris */}
          <mesh position={[0.02, 0, 0.09]}>
            <sphereGeometry args={[0.075, 16, 16]} />
            <meshStandardMaterial color="#3D1A00" />
          </mesh>
          {/* Pupil */}
          <mesh position={[0.025, 0, 0.125]}>
            <sphereGeometry args={[0.04, 12, 12]} />
            <meshStandardMaterial color={dark} />
          </mesh>
          {/* Shine */}
          <mesh position={[0.055, 0.035, 0.135]}>
            <sphereGeometry args={[0.02, 8, 8]} />
            <meshStandardMaterial color={white} emissive={white} emissiveIntensity={1} />
          </mesh>
          {/* Eyelid for blinking */}
          <mesh ref={lidLeftRef} position={[0, 0.06, 0.06]} scale={[1, 0.001, 1]}>
            <sphereGeometry args={[0.135, 20, 10, 0, Math.PI * 2, 0, Math.PI * 0.6]} />
            <meshStandardMaterial color={green} />
          </mesh>
          {/* Eyebrow */}
          <mesh position={[-0.01, 0.14, 0.08]} rotation={[0, 0, -0.25]}>
        ...
        <group position={[-0.19, 0.17, 0.31]}>
          ...
          <mesh ref={lidRightRef} position={[0, 0.06, 0.06]} scale={[1, 0.001, 1]}>
            <sphereGeometry args={[0.135, 20, 10, 0, Math.PI * 2, 0, Math.PI * 0.6]} />
            <meshStandardMaterial color={green} />
          </mesh>
          <mesh position={[0.01, 0.14, 0.08]} rotation={[0, 0, 0.25]}>
        ...
        {/* Rosy cheeks */}
        <mesh position={[0.32, 0.02, 0.26]}>
          <sphereGeometry args={[0.085, 16, 16]} />
          <meshStandardMaterial color={rosy} transparent opacity={0.65} roughness={1} />
        </mesh>
        <mesh position={[-0.32, 0.02, 0.26]}>
          <sphereGeometry args={[0.085, 16, 16]} />
          <meshStandardMaterial color={rosy} transparent opacity={0.65} roughness={1} />

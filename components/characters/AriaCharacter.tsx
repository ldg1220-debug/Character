...
// Aria — warm, graceful female tutor
export default function AriaCharacter({ audioVolume, isSpeaking, emotion }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    // Head graceful sway + nod
    if (headRef.current) {
      const nodTarget = isSpeaking ? Math.sin(ti * cfg.nodSpeed) * 0.04 : 0;
      headRef.current.rotation.x = THREE.MathUtils.lerp(headRef.current.rotation.x, nodTarget, 0.08);
      headRef.current.rotation.z = Math.sin(ti * cfg.idleSpeed * 0.7) * cfg.swayAmp;
      // Gentle tilt when speaking
      const tiltTarget = isSpeaking ? 0.06 : 0;
      headRef.current.rotation.z = THREE.MathUtils.lerp(headRef.current.rotation.z, tiltTarget, 0.05)
        + Math.sin(ti * cfg.idleSpeed * 0.7) * cfg.swayAmp;
    }

    // Explanatory arm gesture when speaking
  ...
  const purple = "#C084FC";
  const deepPurple = "#7C3AED";
  const lightPurple = "#DDA9FF";
  const pink = "#EC4899";
  const rosy = "#F0ABFC";
  const white = "#FFFFFF";
  const dark = "#1a1a1a";
  ...
  return (
    <group ref={rootRef} position={[0, -0.5, 0]}>
      ...
      <group position={[0, 0, 0]}>
        ...
        <mesh scale={[0.88, 1.1, 0.82]}>
          <sphereGeometry args={[0.38, 32, 32]} />
          <meshStandardMaterial color={purple} roughness={0.7} />
        </mesh>
        {/* Chest highlight */}
        <mesh position={[0, 0.06, 0.27]} scale={[0.9, 0.9, 0.5]}>
          <sphereGeometry args={[0.18, 20, 20]} />
          <meshStandardMaterial color={lightPurple} roughness={0.8} transparent opacity={0.5} />
        </mesh>

        {/* Left arm (gesturing) */}
        <group ref={leftArmRef} position={[0.34, 0.16, 0.05]}>
          <mesh rotation={[0.1, 0, 0.3]}>
      ...
      <group ref={headRef} position={[0, 0.62, 0]}>
        ...
        <mesh position={[0, 0.35, -0.06]}>
          <sphereGeometry args={[0.18, 20, 20]} />
          <meshStandardMaterial color={deepPurple} roughness={0.6} />
        </mesh>
        {/* Hair sides flow */}
        <mesh position={[0.28, 0.15, -0.15]} rotation={[0.2, 0.3, 0.5]} scale={[0.7, 1, 0.7]}>
        ...
        <group position={[0.16, 0.1, 0.3]}>
          <mesh scale={[1.1, 1.2, 1]}>
            <sphereGeometry args={[0.11, 20, 20]} />
            <meshStandardMaterial color={white} roughness={0.1} />
          </mesh>
          {/* Iris */}
          <mesh position={[0.015, 0, 0.085]}>
            <sphereGeometry args={[0.065, 16, 16]} />
            <meshStandardMaterial color={deepPurple} />
          </mesh>
          {/* Pupil */}
          <mesh position={[0.018, 0, 0.112]}>
            <sphereGeometry args={[0.035, 12, 12]} />
            <meshStandardMaterial color={dark} />
          </mesh>
          {/* Shine */}
          <mesh position={[0.04, 0.03, 0.122]}>
            <sphereGeometry args={[0.016, 8, 8]} />
            <meshStandardMaterial color={white} emissive={white} emissiveIntensity={1} />
          </mesh>
          {/* Eyelashes — small dark strips on top */}
          {[-0.06, -0.02, 0.02, 0.06].map((x, i) => (
            <mesh key={i} position={[x, 0.1, 0.06]} rotation={[0, 0, x * 0.8]}>
              <boxGeometry args={[0.012, 0.04, 0.01]} />
              <meshStandardMaterial color={dark} />
            </mesh>
          ))}
          {/* Eyelid */}
          <mesh ref={lidLeftRef} position={[0, 0.055, 0.055]} scale={[1, 0.001, 1]}>
            <sphereGeometry args={[0.115, 20, 10, 0, Math.PI * 2, 0, Math.PI * 0.6]} />
        ...
        <group position={[-0.16, 0.1, 0.3]}>
          ...
          <mesh ref={lidRightRef} position={[0, 0.055, 0.055]} scale={[1, 0.001, 1]}>
            <sphereGeometry args={[0.115, 20, 10, 0, Math.PI * 2, 0, Math.PI * 0.6]} />
            <meshStandardMaterial color={purple} />
          </mesh>
        </group>
        ...
        {/* Rosy cheeks */}
        <mesh position={[0.27, -0.02, 0.25]}>
          <sphereGeometry args={[0.075, 16, 16]} />
          <meshStandardMaterial color={rosy} transparent opacity={0.55} roughness={1} />
        </mesh>
        <mesh position={[-0.27, -0.02, 0.25]}>
          <sphereGeometry args={[0.075, 16, 16]} />
          <meshStandardMaterial color={rosy} transparent opacity={0.55} roughness={1} />
